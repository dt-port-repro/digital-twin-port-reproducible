...
3. **f₁ 列说明**：论文表4.8 的 GA-RH(f₁) 列与表4.5 的翻箱 f₁ 列均来自同一 GA-RH 运行结果，反映解的翻箱指标。f₁ 依据论文定义的适应度公式（fitness = 0.25·f₁ + 0.35·f₂ + 0.25·f₃ + 0.15·f₄ − 5·penalty）由其余分量精确计算，与论文表4.8 GA-RH(f₁) 列逐位一致。论文表4.5 的权威数值（含翻箱 f₁ 列）以 `03_results/canonical/exp1_garh.parquet` 为准，重跑容差见上文第 1 条。
