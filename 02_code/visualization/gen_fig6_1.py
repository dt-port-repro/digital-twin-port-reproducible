"""Generate Fig 6.2 - Normal scenario config comparison
...
reg = sim[sim.scenario=='常规作业']

configs = ['A\n(基线)', 'B\n(纯配载)', 'C\n(组合)', 'D\n(协同)']
labels_short = ['A', 'B', 'C', 'D']

a = reg[reg.config=='A'][['turnaround_h','reshuffle_pct','equip_util_pct']].mean()
turnaround = [reg[reg.config==c]['turnaround_h'].mean() for c in ['A','B','C','D']]
reshuffle = [reg[reg.config==c]['reshuffle_pct'].mean() for c in ['A','B','C','D']]
equip_util = [reg[reg.config==c]['equip_util_pct'].mean() for c in ['A','B','C','D']]
...
# Save (PNG + SVG双格式)
plt.savefig(OUT / 'fig6_2_masdes_normal.png', dpi=200, bbox_inches='tight', facecolor='white')
plt.savefig(OUT / 'fig6_2_masdes_normal.svg', dpi=200, bbox_inches='tight', facecolor='white')
plt.close()
print(f'✅ fig6_2_masdes_normal.png')
