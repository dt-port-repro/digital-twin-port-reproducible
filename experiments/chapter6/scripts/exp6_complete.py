...
for ename, bpn, pop in test_ships:
    ...
    for run in range(3):
        ...
        print(f'  策略A run {run}: fitness={res_a["best_fitness"]:.4f} ✅')
    
    # ── 策略C: 近似最优（只补APESP，其他跳过）──
    if ename != 'APESP':
        continue
    ...
    if done_c > 0:
        ...
        print(f'{ename} 策略C: gen=100, pop=80 启动...')
        t0 = time.time()
        opt_c = grh.GARHOptimizer(grh.VesselProblem(ename, bpn, ship.copy(), mb, pd.Series({'max_teu':teu,'e_vessel_name':ename})),
                                  pop_size=80, generations=100)
        ...
        pd.DataFrame(all_results).to_parquet(OUT_FILE, index=False)
        print(f'  策略C: fitness={res_c["best_fitness"]:.4f} ✅')
