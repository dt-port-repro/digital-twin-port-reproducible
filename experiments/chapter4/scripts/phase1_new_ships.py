...
for i,(ename,bpn) in enumerate(new_ships):
    ...
    row={'vessel_code':ename,'n_containers':n,'n_pod':npod,'n_slots':prob.n_slot,'max_teu':teu,
         ...
         'time_s':round(el,1),'n_gen':80,'n_pop':60,'method':'GA-RH','convergence_start':hist[0],'convergence_end':hist[-1]}
    results.append(row)
    print(f'[{i+1}/10] {ename} {n}箱 fitness={row["fitness"]:.4f} {el:.0f}s')

pd.DataFrame(results).to_parquet(OUT/'ga_rh_results'/'phase1_new_ships.parquet',index=False)
print(f'\n✅ Phase1 done: {len(results)} ships saved')
